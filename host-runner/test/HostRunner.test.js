...
test('runGitAction: многокоммитная ветка сдачи → в main доезжают ОБЕ дельты', async (t) => {
  const dir = initRepo(t);
  // Программист сдаёт дельту НЕСКОЛЬКИМИ коммитами в ветке worktree.
  git(dir, ['checkout', '--quiet', '-b', 'programmer/PROJECT_2/host-runner']);
  // Первый прогон: основная работа (упал по max turns, но успел закоммитить).
  writeFileSync(path.join(dir, 'main-work.js'), 'export const core = 42;\n');
  git(dir, ['add', '-A']);
  git(dir, ['commit', '--quiet', '-m', 'programmer: основная работа']);
  // Второй прогон: докоммитил мелочь; deliveredCommit = ТОЛЬКО этот tip.
  writeFileSync(path.join(dir, 'tweak.txt'), 'small\n');
  git(dir, ['add', '-A']);
  git(dir, ['commit', '--quiet', '-m', 'programmer: мелкая правка']);
  const deliveredCommit = git(dir, ['rev-parse', 'HEAD']).trim();
  git(dir, ['checkout', '--quiet', 'main']); // main пока без обеих дельт
  ...
  const res = await runGitAction(
    {
      ...
      title: 'Миграция',
      worktreeBranch: 'programmer/PROJECT_2/host-runner',
      deliveredCommit, // подсказка = tip (последняя мелочь)
      changedFiles: ['main-work.js', 'tweak.txt'],
    },
    { repoRoot: dir },
  );
  ...
  assert.equal(res.success, true);
  assert.ok(res.output.commit, 'должен быть коммит интеграции в main');
  // ОБЕ дельты реально в main — не только последняя (deliveredCommit).
  assert.doesNotThrow(() => git(dir, ['cat-file', '-e', 'main:main-work.js']), 'основная работа должна доехать до main');
  assert.doesNotThrow(() => git(dir, ['cat-file', '-e', 'main:tweak.txt']), 'мелкая правка должна доехать до main');
  assert.equal(git(dir, ['show', 'main:main-work.js']), 'export const core = 42;\n');
  assert.equal(git(dir, ['show', 'main:tweak.txt']), 'small\n');
  const log = git(dir, ['log', '--oneline', 'main']);
  assert.ok(log.includes('основная работа'), 'коммит основной работы должен быть в истории main');
  assert.ok(log.includes('мелкая правка'), 'коммит мелкой правки должен быть в истории main');
  // files в output покрывают ВЕСЬ интегрированный диапазон, а не только последний коммит.
  assert.deepEqual([...res.output.files].sort(), ['main-work.js', 'tweak.txt']);
});

test('runGitAction: грязный ОСНОВНОЙ tree с ДУБЛЕМ дельты (наследие Pipeline Service) → интеграция проходит, дубль унесён в stash', async (t) => {
  const dir = initRepo(t);
  // Программист сдаёт дельту коммитом: НОВЫЙ файл + модификация существующего.
  git(dir, ['checkout', '--quiet', '-b', 'programmer/PROJECT_2/host-runner']);
  writeFileSync(path.join(dir, 'feature.js'), 'export const x = 1;\n');
  writeFileSync(path.join(dir, 'README.md'), 'base-modified\n');
  git(dir, ['add', '-A']);
  git(dir, ['commit', '--quiet', '-m', 'programmer: task delta']);
  const deliveredCommit = git(dir, ['rev-parse', 'HEAD']).trim();
  git(dir, ['checkout', '--quiet', 'main']); // main пока без дельты

  // Эмуляция Pipeline Service: тот же дубль дельты применён в ОСНОВНОМ дереве и
  // НЕ закоммичен (feature.js untracked, README.md modified) — ровно тот дубль,
  // на котором раньше детерминированно падал cherry-pick GI.
  writeFileSync(path.join(dir, 'feature.js'), 'export const x = 1;\n');
  writeFileSync(path.join(dir, 'README.md'), 'base-modified\n');
  assert.notEqual(git(dir, ['status', '--porcelain']).trim(), '', 'дерево должно быть грязным дублем');
  ...
  const res = await runGitAction(
    {
      ...
      worktreeBranch: 'programmer/PROJECT_2/host-runner',
      deliveredCommit,
      changedFiles: ['feature.js', 'README.md'],
    },
    { repoRoot: dir },
  );

  assert.equal(res.success, true, 'интеграция должна пройти, а не BLOCKED');
  assert.ok(res.output.commit, 'должен быть коммит интеграции в main');
  // Дельта реально в main.
  assert.equal(git(dir, ['show', 'main:feature.js']), 'export const x = 1;\n');
  assert.equal(git(dir, ['show', 'main:README.md']), 'base-modified\n');
  // Дубль унесён и снят: после успешной интеграции autostash дропнут, дерево чистое.
  assert.equal(git(dir, ['stash', 'list']).trim(), '', 'autostash должен быть дропнут после успеха');
  assert.equal(git(dir, ['status', '--porcelain']).trim(), '', 'рабочее дерево чистое (дубль унесён, дельта закоммичена)');
});

test('runGitAction: грязный tree с ДРУГИМ содержимым тех же путей (чужая работа) → провал, файлы не тронуты', async (t) => {
  const dir = initRepo(t);
  // Ветка сдачи модифицирует README.md на СВОЁ содержимое.
  git(dir, ['checkout', '--quiet', '-b', 'programmer/PROJECT_2/host-runner']);
  writeFileSync(path.join(dir, 'README.md'), 'delta-from-branch\n');
  git(dir, ['add', '-A']);
  git(dir, ['commit', '--quiet', '-m', 'programmer: task delta']);
  const deliveredCommit = git(dir, ['rev-parse', 'HEAD']).trim();
  git(dir, ['checkout', '--quiet', 'main']);

  // В ОСНОВНОМ дереве незакоммичена ЧУЖАЯ работа по тому же пути (иное содержимое,
  // НЕ дубль дельты) — её нельзя молча затирать.
  writeFileSync(path.join(dir, 'README.md'), 'someone-elses-unrelated-work\n');
  ...
  assert.equal(res.success, false, 'должен быть честный провал, а не тихая перезапись');
  assert.equal(res.output.note, 'dirty_worktree_conflict');
  assert.match(res.output.error, /README\.md/, 'диагностика должна называть конфликтный путь');
  assert.deepEqual(res.output.mismatchedPaths, ['README.md']);
  // Файл на диске НЕ тронут — чужая работа цела, ничего не унесено в stash.
  assert.equal(
    readFileSync(path.join(dir, 'README.md'), 'utf8'),
    'someone-elses-unrelated-work\n',
    'чужое содержимое не должно быть затёрто',
  );
  assert.equal(git(dir, ['stash', 'list']).trim(), '', 'stash не создаётся при несовпадении');
  // Дельта НЕ доехала до main (интеграция не состоялась).
  assert.equal(git(dir, ['show', 'main:README.md']), 'base\n', 'main остаётся с исходным README');
});
