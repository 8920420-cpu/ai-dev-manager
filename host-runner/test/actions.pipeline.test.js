...
test('worktree-сдача → build+deploy ВЫПОЛНЯЮТСЯ в изоляции (рестарт стека), пропущен только smoke', async (t) => {
  const root = tmpDir(t);
  gitIn(root, ['init', '--quiet']);
  gitIn(root, ['config', 'user.email', 'test@local']);
  gitIn(root, ['config', 'user.name', 'test']);
  ...
  // PIPELINE-LOCAL-REBUILD-001: тесты + build + deploy выполняются (стек пересобран
  // и перезапущен из доставленного коммита); порядок стадий сохранён.
  assert.deepEqual(ran, ['npm test', 'docker compose build', 'docker compose up -d']);
  // Пропущен только smoke (up -d --wait) — финальную проверку делает пользователь.
  const stages = Object.fromEntries(
    result.output.summary.actions.map((a) => [a.stage, a]),
  );
  assert.equal(stages.build.status, 'success');
