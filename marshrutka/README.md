...
- бэкенд №1: **72.56.73.96** (timeweb-vpn: nginx `k3s-ingress` слушает 80+30080 →
  upstream 10.10.8.8/10.10.8.9:30080 через awg0) — рабочий;
- бэкенд №2: **195.98.86.63** (базовая, giga: DNAT `ip static tcp ISP 30080
  192.168.1.157 30080` → нода k3s; сам проброс работает — проверен с VPS), но
  health-check'и держат его в down: трафик LB (Timeweb-Москва) → офисный ISP
  не доходит, хотя из Амстердама тот же порт доступен. Асимметричная сетевая
  проблема вне нашего контроля — кандидат на тикет в Timeweb; когда починят,
  бэкенд поднимется сам;
- родной floating IP LB **85.198.81.20 был битым**: ICMP отвечал отовсюду, а
  ...
  08.07.2026 заменён через API floating-ips (unbind/bind, старый удалён) на
  186.246.1.150 — TCP из 12/12 точек. Симптом на будущее: «health зелёный,
  а сайт не открывается» → `check-host.net/check-tcp`;
