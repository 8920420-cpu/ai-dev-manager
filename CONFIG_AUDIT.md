...
| `RUNNER_HOST_TIMEOUT_MS` (орфан host-ролей) | **40 мин** (`db.js`) | compose/.env `2400000` (40 мин) | 40 мин | `db.js` `HOST_TIMEOUT_MS` (жнецы `resetStaleClaims`/`reapOrphanRunningRuns`) | host-роль реапилась по общему 10-мин → срез посреди docker-сборки | ✅ HOST-ORPHAN-TIMEOUT-001: отдельный таймаут + событие `host_orphan_timeout` |
...
`RUNNER_CLAUDE_TIMEOUT_MS` (25 мин) > `PROGRAMMER_TASK_TIMEOUT_MS` (20 мин). Для
host-ролей: `RUNNER_HOST_TIMEOUT_MS` (40 мин) > самой долгой ожидаемой docker-сборки/
коммита — иначе живой host-прогон срежется посреди `docker compose build`
(HOST-ORPHAN-TIMEOUT-001). Оба жнеца ветвят возраст CASE'ом по коду роли и при
принудительном возврате host-роли пишут событие `reason=host_orphan_timeout`
с `roleCode` и `hungMs` (сколько висела).
