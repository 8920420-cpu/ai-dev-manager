...
- Если Claude Code нашёл чужие незакоммиченные правки, сначала проверить их до своей работы:
  посмотреть `git status`, релевантные diff и запустить проверки, подходящие для затронутого проекта.
  Если правки понятны, безопасны и проверки проходят — закоммитить их отдельным коммитом и сразу
  сделать `git push`. Если правки непонятны, рискованные или проверки падают — не перетирать их и
  остановиться с отчётом, что требует решения человека.
- Любой коммит, который создаёт Claude Code, должен быть сразу запушен после успешного коммита.
  Не оставлять локальные незапушенные коммиты, если пользователь явно не попросил обратное.
...
## Скилы агентов и специализация по стеку (AGENT-SKILLS-001 / STACK-SPECIALIZATION-001)

Роль PROGRAMMER остаётся ОДНА. Разделение её на «Go-бэкенд / обмен gRPC /
Next-фронтенд» рассматривалось 04.08.2026 и отвергнуто: параллельность упирается не
в число ролей, а в замок микросервиса (WORK-STACK-001 «один PENDING-элемент на
сервис» + PROGRAMMER-WORKTREE-PER-SERVICE «один активный CODING на сервис»), поэтому
три роли дали бы узлы графа с бэкфиллом по ~22 проектам, три демона и разрыв KPI
роли на несравнимые ряды — без выигрыша в скорости. Контракт proto при этом не
«третий равный», а барьер: очерёдность «владелец контракта раньше потребителей» уже
живёт в PROGRAMMER-CONTRACT-BARRIER-001.

Специализация сделана ДАННЫМИ:

- Архитектор/Mini Architect проставляют `stack` (`go` | `proto` | `next`) — по
  сервису внутри `work_items` и общий по задаче (миграция `0065`, поле НЕ
  обязательное).
- `claimNextClaudeTaskTx` читает его из `tasks.data_card` (`resolveTaskStack` в
  `programmerClaim.js`) и кладёт в задачу раннера полем `stack`.
- Раннер (`programmer-runner/src/skillProfiles.js`) по стеку подаёт агенту профиль
  скилов; стек не задан — определяет его эвристикой по путям файлов задачи
  (`.proto` весит больше `.go`: контракт первичен).

**Скилы едут ЛОКАЛЬНЫМ ПЛАГИНОМ `agent-skills/`** (манифест
`.claude-plugin/plugin.json`, 21 скил, канонические имена
`orchestrator-skills:<имя>`), который раннеры передают в SDK через
`options.plugins` + белый список `options.skills`. Почему не `.claude/skills`:
программист работает в worktree ЧУЖОГО репозитория, а рассуждающие роли — с
`settingSources: []` (COLDSTART-MCP-ISOLATION-001), поэтому ни пользовательские, ни
проектные скилы хоста в этих прогонах не читаются вовсе. Плагин подаётся абсолютным
путём, `skipMcpDiscovery: true` (изоляция MCP не ослабляется), и версионируется
вместе с оркестратором.

- Программист: профиль по стеку + общие (`security-reviewer`,
  `database-migration-engineer`, `observability-engineer`). Подсказка со списком
  идёт в промпт ПЕРЕД правилами исполнения (иначе скил «доступен, но не замечен»).
- Рассуждающие роли: профиль по коду роли (`ARCHITECT` → `system-architect`,
  `TASK_REVIEWER` → `go-code-review`/`frontend-code-review`, `FAILURE_ANALYST` →
  тестовые, инфра-роли → `release-engineer`/`observability-engineer`…). Подсказка
  кладётся в СТАТИЧНУЮ часть промпта — она зависит только от роли и не ломает
  кэш-префикс (PROMPT-CACHE-001). Роли без профиля (Приёмщик, Router) не получают
  ничего.
- Клапаны: `AGENT_SKILLS=0` — выключить целиком; `AGENT_SKILLS_DIR` — другой каталог
  плагина. Нет каталога/пустой профиль → опции пустые, прогон идёт как раньше.
- Добавление скила: каталог в `agent-skills/skills/`, путь в манифест, имя в нужный
  профиль `skillProfiles.js` (профиль — белый список, иначе скил невидим), затем
  рестарт демонов.
