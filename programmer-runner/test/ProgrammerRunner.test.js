...
import { ProgrammerRunner, buildCompletionBody, parseProviderResetAt } from '../src/ProgrammerRunner.js';
...
test('provider limit: session limit ставит cooldown и следующий tick не делает claim', async () => {
  const clock = new Date('2026-07-10T01:10:00+03:00').getTime();
  let claims = 0;
  const http = fakeHttp({ claimReturn: () => { claims += 1; return { task: sampleTask() }; } });
  const runAgent = async () => {
    throw new Error("Claude Code returned an error result: You've hit your session limit · resets 6:50am (Europe/Moscow)");
  };
  const runner = new ProgrammerRunner({
    http, runAgent, log: silent, providerCooldownMs: 60 * 60_000, now: () => clock,
  });
  ...
  assert.equal(claims, 1, 'в cooldown новые задачи не клеймятся');
});

test('parseProviderResetAt: resets 6:50am парсится как ближайшее будущее', () => {
  const now = new Date('2026-07-10T01:10:00+03:00').getTime();
  const reset = parseProviderResetAt("You've hit your session limit · resets 6:50am (Europe/Moscow)", now);
  assert.equal(new Date(reset).getHours(), 6);
  assert.equal(new Date(reset).getMinutes(), 50);
  assert.ok(reset > now);
});
