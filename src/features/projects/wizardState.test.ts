...
describe('wizardReducer / пресеты P0.1', () => {
  ...
  it('buildStateFromProject: сохраняет явный enabled и бэкафиллит код роли', () => {
    const project: Project = {
      ...
      stages: [
        { id: 's1', name: 'Scanner', roleIds: ['r1'], enabled: true, scanPath: '/old' },
        { id: 's2', name: 'Programmer', roleIds: ['r2'], enabled: false },
      ],
      updatedAt: '2025-01-01T00:00:00.000Z',
      createdAt: '2025-01-01T00:00:00.000Z',
    };
    const state = buildStateFromProject(project);
    expect(state.stages.map((s) => s.enabled)).toEqual([true, false]);
