...
роллапом после того, как все дети терминальны. Эпик закрывается `DONE` только
если фактические дети покрывают весь заявленный список сервисов
(`data_card.planned_services`, JOIN-PLANNED-COVERAGE-001) — иначе `BLOCKED` с
перечнем недостающих сервисов (возврат Архитектору). При 0 или 1 сервисе шаг
остаётся линейным (одна задача Programmer; 0 сервисов → `BLOCKED`).
...
8. **Расщепление Архитектора.** При `FORWARD`-вердикте с ≥2 затронутыми
   ...
   закрывается роллапом после терминальности всех детей и только при полном
   покрытии заявленного списка сервисов (`data_card.planned_services`), иначе
   `BLOCKED` c `reason='epic_missing_services'` (ADR-013,
   JOIN-PLANNED-COVERAGE-001).
