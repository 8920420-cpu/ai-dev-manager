...
192.168.1.211 (kubeconfig указывает на LAN-адрес первой ноды, SAN включает
и LAN-, и WG-адреса).
...
peers с endpoint'ами LAN-адресов и keepalive 25). После правки ключа/конфига
...
192.168.5.N/32), с удалённых площадок endpoint = LAN-адрес через site-to-site
маршрутизацию. flannel при этом строит собственный WG-mesh по LAN-адресам
(эндпоинты 51820) — pod-трафик не ходит через wg-cluster, двойного
шифрования нет.
