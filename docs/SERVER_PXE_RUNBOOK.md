...
## Кластерная сеть 192.168.5.0/24 (wg-cluster)

Кластер живёт в собственной подсети **192.168.5.0/24** поверх WireGuard-mesh
(интерфейс `wg-cluster`, UDP **51830** — 51820 занят flannel'ом), чтобы не
пересекаться с рабочими сетями площадок. Управление — по-прежнему с
192.168.1.211 (kubeconfig ходит на `https://k3s-1.ps.lan:6443` — имя из
внутренней DNS-зоны ps.lan на всех трёх Keenetic, добавлено в tls-san юнитов
k3s обеих нод 08.07.2026; SAN включает также LAN-, WG- и белые адреса).

| Нода | LAN (ssh) | wg-cluster |
|---|---|---|
| node-1a7af6 (железо, Базовая) | 192.168.1.157 | 192.168.5.1 |
| node-01d306 «ПС-2» (ВМ на 211) | 192.168.1.213 | 192.168.5.2 |
| node-029506 (ВМ на Каскаде) | 172.31.x — NAT Hyper-V, ssh через 192.168.5.3 | 192.168.5.3 |

На каждой ноде: приватный ключ в `/etc/wireguard/cluster.key`
(root:systemd-network 640; каталог `/etc/wireguard` — 750, иначе
systemd-networkd молча игнорирует netdev: «Failed to read private key»),
конфиг — `/etc/netplan/61-wg-cluster.yaml` (mode wireguard, mtu 1340,
keepalive 25). Endpoint'ы — **dial-out схема по именам зоны ps.lan**
(08.07.2026): у каждой пары один звонящий — 5.1 звонит 5.2 по
`k3s-2.ps.lan:51830`, будущая 5.3 звонит обеим (`wg-join.sh`); у принимающей
стороны endpoint для звонящего НЕ прописан — адрес учится по роумингу
WireGuard из подписанных пакетов. Переезд ноды на другую площадку: обновить
её `ip host` записи на роутерах — конфиги нод не трогаются. На нодах зона
прибита к giga drop-in'ом `/etc/systemd/resolved.conf.d/ps-lan.conf`
(`DNS=192.168.1.1`, `Domains=~ps.lan`). После правки ключа/конфига
нужен `systemctl restart systemd-networkd` — голый `netplan apply` не
пересоздаёт netdev, который networkd уже отбросил. Новую ноду подключать:
сгенерировать ключ, добавить её peer'ом на все существующие ноды (allowed-ips
192.168.5.N/32, без endpoint — новая нода звонит сама). flannel при этом
строит собственный WG-mesh по LAN-адресам (эндпоинты 51820) — pod-трафик не
ходит через wg-cluster, двойного шифрования нет; endpoint'ы flannel берёт из
аннотаций нод и обновляет сам после старта k3s на новом адресе.

**MTU flannel прибит вручную** (08.07.2026). Flannel MTU сам НЕ подбирает —
берёт от интерфейса дефолт-маршрута (eth0=1500 → flannel-wg 1420), а реальный
PMTU межплощадочного пути до Каскада ~1112: pod-пакеты крупнее ~1100 Б молча
терялись (вложенный WG, ICMP frag-needed не возвращается). Фикс: на всех
нодах `/etc/rancher/k3s/flannel-netconf.json` — копия штатного net-conf с
`Backend.MTU: 1080` (это бюджет **внешнего** пакета; интерфейс flannel-wg
получает 1080−80=**1000**) — и `/etc/rancher/k3s/config.yaml` со строкой
`flannel-conf: /etc/rancher/k3s/flannel-netconf.json`; применяется рестартом
k3s/k3s-agent. `provision-k3s.sh` пишет оба файла сам (env
`K3S_FLANNEL_MTU`, по умолчанию 1080). После смены MTU мост `cni0` обновлён
руками (`ip link set cni0 mtu 1000`) — сам он не пересоздаётся; поды,
созданные до фикса, остались с veth 1420 — локальный PMTUD это разруливает,
но при пересоздании они получат 1000 из `/run/flannel/subnet.env`.
...
# кластер в отдельной WG-сети: пары ssh_ip:node_ip
...
1. пишет `/etc/rancher/k3s/registries.yaml` — доверие к
   `http://$PXE_SERVER_IP:5000` (локальный registry) — и
   `flannel-netconf.json` + `config.yaml` (жёсткий MTU flannel,
   env `K3S_FLANNEL_MTU`, см. «Кластерная сеть»);
...
mesh (обычный vxlan упирался бы во фрагментацию на межплощадочных туннелях).
MTU flannel сам НЕ подбирает — задаётся явно через `--flannel-conf`
(см. «Кластерная сеть 192.168.5.0/24»). Белые IP площадок в
...
Известные белые IP площадок (дублируются в `.env` → `K3S_TLS_SAN_EXTRA`):

| Площадка | Роутер | Белый IP |
|---|---|---|
| Барикадная | psvrn.keenetic.link (Voyager Pro) | **188.235.1.207** |
| Каскад | kaskadvrn.keenetic.link | **5.187.76.240** |
| Базовая (211/157) | giga-lan 192.168.1.1 | 195.98.86.63 |
