...
test('после одного Reviewer→Programmer возврата сдача Programmer пропускает Reviewer и идёт в Pipeline', async () => {
  __resetRoleFieldsCacheForTests();
  const c = fakeClient([
    ...scannerBaseRules(),
    {
      re: /FROM task_events e\s+JOIN roles r[\s\S]*r\.code = 'TASK_REVIEWER'/,
      reply: { rowCount: 1, rows: [{ n: 1 }] },
    },
    {
...
// Архитектор/work_stack) сдача Programmer'а ДОЛЖНА перенести позицию в графе на узел
// ревьюера. Регрессия 10.07: current_stage_key застревал на узле Programmer, а guard
// захвата claimLlmRoleTask (ps.stage_key = current_stage_key) для этапа ревьюера не
// совпадал — задача навсегда зависала в REVIEW, не взятая ни одним движком.
test('graph-задача: сдача Programmer переносит current_stage_key на узел ревьюера', async () => {
  __resetRoleFieldsCacheForTests();
  const rules = [
    { re: /SELECT id, code FROM projects/, reply: { rowCount: 1, rows: [{ id: 'proj-1', code: 'PS' }] } },
    {
      ...
      reply: { rowCount: 1, rows: [{
        ...
        service_code: 'Catalog_Service', reviewer_role_id: 'rev-1',
        current_role_id: 'role-prog', current_role_code: 'PROGRAMMER',
        current_stage_key: 'prog-key', task_kind: 'service',
      }] },
    },
    { re: /INSERT INTO scanner_dispatches/, reply: { rowCount: 1, rows: [{ id: 'disp-1' }] } },
    // Маршрут проекта: Programmer(CODING, prog-key) → Task Reviewer(REVIEW, rev-key).
    { re: /FROM project_stages WHERE project_id/, reply: { rowCount: 2, rows: [
      { id: 'st-prog', position: 0, enabled: true, task_status: 'CODING', stage_key: 'prog-key' },
      { id: 'st-rev', position: 1, enabled: true, task_status: 'REVIEW', stage_key: 'rev-key' },
    ] } },
    { re: /FROM project_stage_roles psr JOIN roles r/, reply: { rowCount: 2, rows: [
      { stage_id: 'st-prog', code: 'PROGRAMMER', position: 0 },
      { stage_id: 'st-rev', code: 'TASK_REVIEWER', position: 0 },
    ] } },
    { re: /SELECT id FROM roles WHERE code/, reply: { rowCount: 1, rows: [{ id: 'rev-1' }] } },
    { re: /to_regclass\('public\.role_fields'\)/, reply: { rowCount: 1, rows: [{ t: 'role_fields' }] } },
    { re: /FROM role_fields rf/, reply: { rowCount: 1, rows: [{ direction: 'out', required: true, key: 'diff' }] } },
  ];
  const c = fakeClient(rules);
  const res = await acceptScannerCompletionTx(c, scannerPayload({ fields: { diff: 'patch' } }));
  assert.equal(res.accepted, true);
  assert.equal(res.nextRole, 'TASK_REVIEWER');
  const upd = c.calls.find((q) => /UPDATE tasks\s+SET status/.test(q.sql));
  assert.ok(upd, 'задача продвинута');
  assert.equal(upd.params[1], 'REVIEW', 'статус → REVIEW');
  assert.ok(/current_stage_key\s*=\s*\$5::uuid/.test(upd.sql), 'UPDATE переносит current_stage_key');
  assert.equal(upd.params[4], 'rev-key', 'current_stage_key перенесён на узел ревьюера');
});
