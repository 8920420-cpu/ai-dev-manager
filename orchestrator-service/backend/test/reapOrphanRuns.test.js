...
// BOOT-RECONCILE-GRACE-001 (требование 1+3): щадящий boot-reconcile гасит только
// RUNNING старше штатного таймаута роли (ageCheck=true → есть возрастной предикат,
// молодые прогоны переживают рестарт), а событие метится деплой-маркером boot.
test('reap (boot, ageCheck+boot): возрастной предикат + аннотация bootReconcile/deployRef', async () => {
  __resetClockGuard();
  const c = fakeClient([
    { re: /EXTRACT\(EPOCH/, reply: { rows: [{ ms: 1_000_000 }], rowCount: 1 } },
    { re: REAP_RE, reply: { rowCount: 1, rows: [] } },
  ...
  const freed = await reapOrphanRunningRuns(c, { ageCheck: true, boot: true, deployRef: 'sha-abc123' });
  assert.equal(freed, 1);

  const main = c.calls.find((q) => REAP_RE.test(q.sql));
  assert.ok(main, 'основной запрос вызван');
  // Grace: молодые прогоны спасены возрастным предикатом (гасим только просроченные).
  assert.ok(/started_at < now\(\)/.test(main.sql), 'boot-реап проверяет возраст (grace для молодых RUNNING)');
  // Событие реапа помечено как boot-реконсиляция + деплой-маркер (диагностика req.3).
  assert.ok(/'bootReconcile', true/.test(main.sql), 'событие помечено bootReconcile=true');
  assert.ok(/'deployRef', \$\d+::text/.test(main.sql), 'в событие подставлен деплой-маркер');
  assert.equal(main.params[main.params.length - 1], 'sha-abc123', 'deployRef передан последним параметром');
});

// deployRef может отсутствовать (APP_CODE_VERSION не задан) — аннотация всё равно
// ставится (bootReconcile=true), deployRef = NULL, запрос не падает.
test('reap (boot): без deployRef аннотация ставится, параметр = null', async () => {
  __resetClockGuard();
  const c = fakeClient([
    { re: /EXTRACT\(EPOCH/, reply: { rows: [{ ms: 1_000_000 }], rowCount: 1 } },
    { re: REAP_RE, reply: { rowCount: 0, rows: [] } },
  ]);
  await reapOrphanRunningRuns(c, { ageCheck: true, boot: true });
  const main = c.calls.find((q) => REAP_RE.test(q.sql));
  assert.ok(/'bootReconcile', true/.test(main.sql), 'bootReconcile=true даже без deployRef');
  assert.equal(main.params[main.params.length - 1], null, 'deployRef = null');
});
