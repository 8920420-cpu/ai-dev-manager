...
// НЕ расщепляется повторно: он уже сфокусирован на своём service_id и идёт одиночным
// путём (ensureArchitectService). Без guard он бесконечно порождал эпик→эпик→эпик.
test('split: у ребёнка (parent_task_id задан) повторного расщепления нет — ведём одиночным путём', async () => {
  const c = fakeClient([
    // ensureArchitectService: у ребёнка service_id уже задан → резолв не нужен, не блок.
    { re: /SELECT service_id FROM tasks WHERE id = \$1/, reply: { rowCount: 1, rows: [{ service_id: 'sidA' }] } },
    // preflight эффективного сервиса ребёнка — валидный путь → ok, идём вперёд.
    { re: /FROM services s JOIN projects p/, reply: {
      rowCount: 1, rows: [{ service_code: 'SvcA', repository_path: 'CRM/SvcA', root_path: 'K:\\no\\such\\host\\root' }],
    } },
    twoServicesRule,
    { re: /FROM roles WHERE code = \$1/, reply: { rowCount: 1, rows: [{ id: 'rProg' }] } },
  ]);

  // Тот же 2-сервисный вердикт, но claimed — split-ребёнок (parent_task_id задан).
  const res = await applyReasoningVerdict(c, architectClaimed({ id: 'child1', parent_task_id: 'epic1' }), {
    route: LINEAR_ROUTE, contract: { outputs: [] }, verdict: splitVerdict(),
    response: '', exchangeId: 'ex1', durationMs: 1,
  });

  const svcInserts = c.calls.filter((q) => /INSERT INTO tasks[\s\S]*'service'[\s\S]*RETURNING id/.test(q.sql));
  assert.equal(svcInserts.length, 0, 'повторного расщепления на детей нет');
  const park = c.calls.find((q) => /UPDATE tasks SET task_kind = 'epic'/.test(q.sql));
  assert.equal(park, undefined, 'ребёнок НЕ паркуется как эпик');
  const splitEvent = c.calls.find((q) => /INSERT INTO task_events/.test(q.sql)
    && typeof q.params?.[3] === 'string' && /architect_service_split/.test(q.params[3]));
  assert.equal(splitEvent, undefined, 'события architect_service_split нет — пошёл одиночным путём');
  // ensureArchitectService реально вызван (резолв service_id ребёнка) — доказывает вход
  // в одиночную ветку вместо split.
  assert.ok(c.calls.some((q) => /SELECT service_id FROM tasks WHERE id = \$1/.test(q.sql)),
    'ensureArchitectService вызван — одиночный путь');
});
