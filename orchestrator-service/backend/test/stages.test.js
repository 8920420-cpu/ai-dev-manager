...
test('validateStages: этап без enabled отклоняется', () => {
  const errors = validateStages([
    { id: 's1', name: 'Разработка', roleCodes: ['PROGRAMMER'], taskStatus: 'CODING' },
  ]);
  assert.ok(errors.some((e) => e.stageId === 's1' && e.code === STAGE_ERROR.ENABLED_REQUIRED));
});

test('validateStages: enabled должен быть boolean', () => {
  for (const value of [undefined, null, 'true', 1, 0]) {
    const errors = validateStages([
      { id: 's1', name: 'Этап', enabled: value, roleCodes: ['SCANNER'], watchDirectory: '/abs', taskStatus: 'CODING' },
    ]);
    assert.ok(
      errors.some((e) => e.stageId === 's1' && e.code === STAGE_ERROR.ENABLED_REQUIRED),
      `enabled=${JSON.stringify(value)} должен быть ошибкой`,
    );
  }
