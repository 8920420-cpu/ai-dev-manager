...
- Назначение «роль→коннектор»: `role_connectors` (role_code → connectors.id).
...
```jsonc
{ "roleCode":"PROGRAMMER", "connectorId":"uuid|null", "updatedAt":"ISO" }
```
...
- `PUT /api/role-connectors` → массовое сохранение `{ assignments:[{roleCode, connectorId|null}] }`.
  `connectorId: null` снимает назначение. Возврат: актуальный список.
