...
Поле `enabled` **необязательно**. Совместимость (LEGACY-STAGE-DEFAULTS): этап
без `enabled` или с любым значением кроме явного `false` трактуется как
**включённый** (`enabled: true`); только явный `false` отключает этап. Это
зеркалит фронтендовый признак «нет галочки „Пропустить“ → включён». Сервер
сохраняет нормализованный boolean, поэтому `GET` всегда возвращает явный `enabled`.
