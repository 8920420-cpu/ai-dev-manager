...
// исполняются вне ИИ (host-runner, файловый сканер, отдельный конвейер Claude
// Code), поэтому назначение им reasoning-движка вводит оператора в заблуждение и
// отклоняется. Источник истины — LLM_ROLE_CODES (roleEngine.js), нормализуем
 ...
 * reasoningRoleCodes — Set кодов рассуждающих ролей (UPPER/TRIM); если передан,
 * назначение коннектора (connectorId!=null) не-reasoning роли отклоняется
 * (ROLE-CONNECTOR-REASONING-ONLY-001). Снятие (connectorId=null) разрешено всем.
 ...
 */
export function normalizeRoleConnectors(input, { validRoleCodes, validConnectorIds, reasoningRoleCodes } = {}) {
  ...
  for (const item of list) {
    ...
    // ROLE-CONNECTOR-REASONING-ONLY-001: назначить движок можно только рассуждающей
    // роли. Снятие (connectorId=null) не ограничиваем — его разрешаем любой роли.
    if (connectorId !== null && reasoningRoleCodes && !reasoningRoleCodes.has(roleCode.toUpperCase())) {
      throw httpError(422, 'role_connector_role_not_reasoning', { code: 'role_connector_role_not_reasoning' });
    }
...
export async function saveRoleConnectors(s, input) {
  return withClient(clientConfig(s), async (c) => {
    ...
    const normalized = normalizeRoleConnectors(input, {
      validRoleCodes,
      validConnectorIds,
