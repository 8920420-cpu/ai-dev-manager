 ...
 * Контракт enabled — совместимость «отсутствует = включён» (LEGACY-STAGE-DEFAULTS):
 * этап без поля или с любым значением кроме явного false считается включённым.
 * Само по себе значение enabled ошибкой быть не может — поэтому отдельного кода
 * ошибки для него нет (зеркалит фронтендовый isStageEnabled = enabled !== false).
 */
export function validateStages(stages, { requireScannerWatch = true } = {}) {
  ...
  for (const stage of list) {
    ...
    // LEGACY-STAGE-DEFAULTS: этап без enabled (или с любым значением кроме явного
    // false) трактуется как включённый — совместимость со старыми записями
    // (absent = true). Только явный false отключает этап.
