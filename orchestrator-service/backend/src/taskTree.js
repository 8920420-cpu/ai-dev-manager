  ...
  return withClient(clientConfig(s), async (c) => {
    ...
    const tasksRes = await c.query(
      `SELECT id, project_id, parent_task_id, title,
              status::text AS status, priority::text AS priority, created_at, data_card
         FROM tasks
        ORDER BY created_at`,
    );
...
function stripNode(node, leaf = false) {
  ...
  // DOCS-DEBT-001: непогашенный документационный долг задачи/подзадачи (или null).
  if (node.docsDebt) out.docsDebt = node.docsDebt;
